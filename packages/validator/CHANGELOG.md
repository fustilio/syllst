# @syllst/validator

## 0.4.10

### Patch Changes

- update config
- Updated dependencies
  - @syllst/processor@0.6.1

## 0.4.9

### Patch Changes

- Updated dependencies
  - @syllst/processor@0.6.0

## 0.4.6

### Patch Changes

- Patch version bump for republishing
- Updated dependencies
  - @syllst/processor@0.5.3

## 0.4.5

### Patch Changes

- Patch version bump for republishing
- Updated dependencies
  - @syllst/processor@0.5.2

## 0.4.3

### Patch Changes

- Updated dependencies
  - @syllst/processor@0.5.0

## 0.4.2

### Patch Changes

- update build config add JS extensions for syllst-processor
- Updated dependencies
  - @syllst/processor@0.4.2

## 0.4.1

### Patch Changes

- @syllst/processor@0.4.1

## 0.4.0

### Minor Changes

- Synchronized version with ecosystem-wide 0.4.0 release
- Validation now benefits from `@syllst/core`'s discriminated union exercise schemas, providing per-type structural error messages

### Patch Changes

- Updated dependencies
  - @syllst/processor@0.4.0

## 0.1.2

### Patch Changes

- Updated dependencies
  - @syllst/processor@0.3.0

## 0.1.1

### Patch Changes

- Updated dependencies
  - @syllst/processor@0.2.2
